...
export const ProcessSection: React.FC = () => {
  ...
  return (
    <section id="process" className="py-24 bg-white relative overflow-hidden">
      {/* Dynamic Background */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-blue-500/5 rounded-full blur-[160px] pointer-events-none" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((s, idx) => {
            ...
            return (
              <div
                key={idx}
                className="group relative rounded-2xl bg-white border border-slate-200/90 p-6 sm:p-7 flex flex-col justify-between hover:border-blue-400 hover:shadow-lg transition-all duration-300"
              >
                <div>
                  {/* Step Number & Tag */}
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-2xl font-extrabold font-mono text-slate-300 group-hover:text-blue-600 transition-colors">
                      {s.stepNumber}
                    </span>
                    <span className="text-[11px] font-mono font-semibold text-blue-700 bg-blue-50 border border-blue-200 px-2.5 py-1 rounded-full">
                      {s.tagline}
                    </span>
                  </div>

                  {/* Icon */}
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${s.color} p-[1px] mb-5 shadow-sm`}>
                    <div className="w-full h-full bg-white rounded-[11px] flex items-center justify-center text-blue-600 group-hover:scale-105 transition-transform">
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100">
                    {s.points.map((pt, pIdx) => (
                      <div key={pIdx} className="flex items-start gap-2 text-[11px] text-slate-700 font-medium">
