...
export const AboutSection: React.FC = () => {
  return (
    <section id="about" className="py-24 bg-[#07090e] relative overflow-hidden">
      {/* Background Radiance */}
      <div className="absolute top-1/3 left-1/4 w-[600px] h-[350px] bg-indigo-600/10 rounded-full blur-[160px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {companyValues.map((val, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-6 rounded-2xl bg-[#0b0f19] border border-slate-800 hover:border-cyan-500/50 transition-all duration-300 flex flex-col justify-between"
              >
                <div>
        ...
        <div className="mb-20">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <h3 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">Executive Leadership</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {leadershipTeam.map((leader, idx) => (
              <div
                key={idx}
                className="p-5 rounded-2xl bg-[#0b0f19] border border-slate-800 text-center flex flex-col items-center hover:border-slate-700 transition-colors"
              >
                <img
                  src={leader.image}
                  alt={leader.name}
                  className="w-24 h-24 rounded-full object-cover mb-4 ring-2 ring-cyan-500/30"
                />
                <h4 className="text-sm font-bold text-white">{leader.name}</h4>
                <div className="text-xs text-cyan-400 font-medium mb-3">{leader.role}</div>
        ...
        <div className="rounded-3xl bg-slate-900/50 border border-slate-800 p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
            ...
            <div className="px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {officeLocations.map((loc) => (
              <div key={loc.city} className="p-4 rounded-xl bg-slate-950/60 border border-slate-800">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-white text-sm">{loc.city}</span>
                  {loc.isHQ && <span className="text-[10px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded font-mono">Global HQ</span>}
                </div>
                <div className="text-xs text-cyan-400 mb-1">{loc.country}</div>
                <div className="text-[11px] text-slate-400 line-clamp-2">{loc.address}</div>
                <div className="text-[11px] text-slate-400 font-mono mt-2">{loc.phone}</div>
              </div>
            ))}
          </div>
        </div>
